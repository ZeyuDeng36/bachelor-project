...
def compute_combined_loss_batched(
    evidence: torch.Tensor, label: torch.Tensor
) -> torch.Tensor:
    """
...
def sort_by_total_evidence(model, dataset, batch_size=128):
    return get_scores_batched(
        model, dataset, compute_total_evidence_batched, batch_size
    )


def sort_by_total_uncertainty(model, dataset, batch_size=128):
    return get_scores_batched(
        model, dataset, compute_total_uncertainty_batched, batch_size
    )


def sort_by_label_evidence(model, dataset, batch_size=128):
    return get_scores_batched(
        model, dataset, compute_label_evidence_batched, batch_size
    )


def sort_by_label_uncertainty(model, dataset, batch_size=128):
    return get_scores_batched(
        model, dataset, compute_label_uncertainty_batched, batch_size
    )
