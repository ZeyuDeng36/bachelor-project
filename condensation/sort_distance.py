...
####################################
def geometric_median_tensor(
    features: torch.Tensor, eps: float = 1e-5, max_iter: int = 500
) -> torch.Tensor:
    """
    ...
    for _ in range(max_iter):
        ...
        new_median = torch.sum(
            features * inv_distances.unsqueeze(1), dim=0
        ) / torch.sum(inv_distances)
...
def compute_class_geometric_medians(
    dataset, batch_size=128, eps: float = 1e-5, max_iter: int = 500
) -> dict:
    """
...
def normalized_distance_sample(
    ...
    if selected_norms.numel() == 0:
        ...
        scale = (
            torch.mean(selected_norms) if mode == "avg" else torch.max(selected_norms)
        )

    return norm_sample / (scale + eps)


def compute_normalized_distance_scores(
    dataset, centroids: dict, batch_size=128, threshold=0.9, mode="max"
) -> list:
    """
    ...
    for idx, (feat, label) in enumerate(zip(ordered_features, ordered_labels)):
        ...
        norm_score = normalized_distance_sample(
            feat, center, class_feats, threshold=threshold, mode=mode
        )
