...
class Trainer:
    def __init__(
        ...
        if optimizer_type == "adam":
            ...
            raise ValueError(
                f"Invalid optimizer type: {optimizer_type}. Valid optimizers: 'adam', 'sgd', 'rmsprop'"
            )
        ...
        if criterion == "crossEntropy":
            ...
            raise ValueError(
                f"Invalid criterion: {criterion}. Valid criteria: 'crossEntropy', 'evidential'"
            )
    ...
    def save_model(self, fileName, train_loss, train_accuracy, val_loss, val_accuracy):
        ...
        with open("models/modelStats.txt", "a") as file:
            file.write(
                f"MODEL:{fileName} , Train Loss: {train_loss:.4f}, Train Accuracy: {train_accuracy:.2f}, Validation Loss: {val_loss:.4f}, Validation Accuracy: {val_accuracy:.2f}%"
                + "\n"
            )
    ...
    def train(self, verbose=False):
        ...
        if self.save != "":
            self.save_model(
                self.save, train_loss, train_accuracy, val_loss, val_accuracy
            )
