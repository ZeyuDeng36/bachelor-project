...
from condensation.select import (
    ...
    balanced_by_score1,
    balanced_by_range,
    select_median_centered,
    select_least_bin,
)
from condensation.sort_entropy import (
    sort_by_log_percentage_entropy,
    sort_by_predictive_entropy,
)
from condensation.sort_evidence import (
    sort_by_total_evidence,
    sort_by_label_evidence,
    sort_by_label_uncertainty,
    sort_by_total_uncertainty,
    sort_by_combined_loss,
)
from condensation.sort_evidence1 import (
    sort_by_total_evidence_dirichlet,
    sort_by_label_belief,
    compute_input_gradient_norm,
)
...
def run_experiments(
    ...
    model1 = (
        initiate_model(model_name, dataset_name, pretrained) if pretrained else None
    )
    ...
    for rate in rates:
        ...
        )  # new untrained model for training
        if rate <= 0:
            trainset_condensed = trainset
        else:
            trainset_condensed_indices = selection_func(
                trainset, 1 - rate, sorted_scores
            )
        ...
        print(filename)
        trainer = Trainer(model, trainset_condensed, testset, save=filename)
...
def run_experiments1(
    ...
    for rate in rates:
        ...
        filename = "-".join(str(p) for p in parts if p is not None)
        trainer = TrainerE(model, trainset_condensed, testset, save=filename)
        trainer.train(verbose=True)
...
def run_experiments2(
    ...
    for rate in rates:
        ...
        parts = [modelStr, dataset_name, scoring_method, selection_method, rate, suffix]
        filename = "-".join(str(p) for p in parts if p is not None)
        trainer = TrainerE1(model, trainset_condensed, testset, save=filename)
        trainer.train(verbose=True)
